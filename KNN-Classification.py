...
test_data3.replace(1.00000000000000e+99, np.nan, inplace=True)
imputer = SimpleImputer(strategy='mean')
train_data3 = pd.DataFrame(imputer.fit_transform(train_data3))
test_data3 = pd.DataFrame(imputer.transform(test_data3))
knn_no_scale = KNeighborsClassifier(n_neighbors=5)
scores_no_scale = cross_val_score(knn_no_scale, train_data3, train_labels3, cv=5)
# Fit on all training data and predict
knn_no_scale.fit(train_data3, train_labels3)
pred_no_scale = knn_no_scale.predict(test_data3)
pipe_scaled = Pipeline([
    ('scaler', StandardScaler()),
    ('knn', KNeighborsClassifier(n_neighbors=5))
])
scores_scaled = cross_val_score(pipe_scaled, train_data3, train_labels3, cv=5)
pipe_scaled.fit(train_data3, train_labels3)
pred_scaled = pipe_scaled.predict(test_data3)
print(f"\nSet 1: KNN without scaling - Mean CV Accuracy: {scores_no_scale.mean():.4f}")
print(f"Set 1: KNN with scaling - Mean CV Accuracy: {scores_scaled.mean():.4f}")